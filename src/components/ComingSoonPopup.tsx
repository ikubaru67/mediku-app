...
export default function ComingSoonPopup({ feature, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose} onKeyDown={(e) => e.key === 'Escape' && onClose()}>
      <div className="bg-white rounded-[32px] p-8 mx-6 max-w-sm w-full shadow-xl animate-pop-in text-center" onClick={e => e.stopPropagation()}>
        ...
        <button ref={btnRef} onClick={onClose} aria-label="Tutup"
