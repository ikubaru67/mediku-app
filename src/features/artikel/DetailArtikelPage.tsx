        ...
        <div className="flex items-center justify-between bg-[#F2F4F7] rounded-2xl p-4 mb-6">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full overflow-hidden bg-[#D8E2FF] flex items-center justify-center shrink-0">
              {foto ? (
                <img src={foto} alt={artikel.penulis} className="w-full h-full object-cover" />
              ) : (
          ...
          <button className="px-4 py-2 border-2 border-[#0059BB] rounded-full text-[#0059BB] text-sm font-semibold focus-visible:ring-2 focus-visible:ring-[#0059BB] focus-visible:outline-none">Ikuti</button>
        </div>

        <div className="w-full rounded-[32px] overflow-hidden mb-6 shadow-[0px_10px_30px_rgba(0,123,255,0.05)]">
          <img src={IMAGES.feed[imgIdx]} alt={artikel.judul} className="w-full h-auto object-cover" />
        </div>
