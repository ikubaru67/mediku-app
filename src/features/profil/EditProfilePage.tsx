    ...
    <div className="min-h-screen bg-[#F7F9FC] pb-8">
      <div className="bg-white rounded-b-[32px] px-5 h-20 flex items-center justify-between shadow-[0px_4px_6px_-4px_rgba(0,0,0,0.10)]">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate(-1)} className="w-10 h-10 rounded-full bg-[#0486F1] flex items-center justify-center">
          ...
          <span className="text-[#0486F1] text-base font-extrabold">Edit Profil</span>
        </div>
        <button onClick={handleSave} disabled={saving}
          className="bg-[#0486F1] text-white text-sm font-semibold rounded-full px-6 py-2">
      ...
      <div className="px-5 pt-8 stagger-fade">
        ...
        <div className="flex justify-center mb-8">
          <div className="relative">
            <div className="w-20 h-20 rounded-full overflow-hidden border-4 border-white shadow-[0px_4px_6px_-4px_rgba(0,0,0,0.10),0px_10px_15px_-3px_rgba(0,0,0,0.10)] bg-gradient-to-br from-[#0059BB] to-[#006D43] flex items-center justify-center">
              {user?.photoURL ? <img src={user.photoURL} alt="" className="w-full h-full object-cover" /> : <span className="text-white font-bold text-xl">{initials}</span>}
            </div>
        ...
        <div className="bg-white rounded-[32px] p-6 mb-4 shadow-[0px_10px_30px_rgba(0,89,187,0.05)] outline-1 outline-[rgba(193,198,215,0.30)]">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-[#414754] text-xs font-medium mb-1 block">Nama Lengkap</label>
            ...
            <div>
              <label className="text-[#414754] text-xs font-medium mb-1 block">Email</label>
              <input type="email" value={form.email} onChange={e => update('email', e.target.value)}
                className="w-full bg-[#F2F4F7] border border-[rgba(193,198,215,0.50)] rounded-full px-4 py-3 outline-none text-sm text-[#191C1E]" />
            </div>
            <div>
              <label className="text-[#414754] text-xs font-medium mb-1 block">Nomor Telepon</label>
              <input type="text" value={form.phone} onChange={e => update('phone', e.target.value)}
                className="w-full bg-[#F2F4F7] border border-[rgba(193,198,215,0.50)] rounded-full px-4 py-3 outline-none text-sm text-[#191C1E]" />
            </div>
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="text-[#414754] text-xs font-medium mb-1 block">Tanggal Lahir</label>
                <input type="date" value={form.tanggalLahir} onChange={e => update('tanggalLahir', e.target.value)}
                  className="w-full bg-[#F2F4F7] border border-[rgba(193,198,215,0.50)] rounded-full px-4 py-3 outline-none text-sm text-[#191C1E]" />
              </div>
              <div className="flex-1">
                <label className="text-[#414754] text-xs font-medium mb-1 block">Jenis Kelamin</label>
                <select value={form.jenisKelamin} onChange={e => update('jenisKelamin', e.target.value)}
        ...
        <div className="bg-white rounded-[32px] p-6 mb-4 shadow-[0px_10px_30px_rgba(0,89,187,0.05)] outline-1 outline-[rgba(193,198,215,0.30)]">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-[#414754] text-xs font-medium mb-1 block">Berat Badan</label>
              <div className="relative">
                <input type="text" value={form.berat} onChange={e => update('berat', e.target.value)}
                  className="w-full bg-[#F2F4F7] border border-[rgba(193,198,215,0.50)] rounded-full px-4 py-3 outline-none text-sm text-[#191C1E]" />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[#414754] text-sm">kg</span>
              </div>
            </div>
            <div>
              <label className="text-[#414754] text-xs font-medium mb-1 block">Tinggi Badan</label>
              <div className="relative">
                <input type="text" value={form.tinggi} onChange={e => update('tinggi', e.target.value)}
                  className="w-full bg-[#F2F4F7] border border-[rgba(193,198,215,0.50)] rounded-full px-4 py-3 outline-none text-sm text-[#191C1E]" />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[#414754] text-sm">cm</span>
              </div>
            </div>
            <div>
              <label className="text-[#414754] text-xs font-medium mb-1 block">Gol. Darah</label>
              <select value={form.goldar} onChange={e => update('goldar', e.target.value)}
                className="w-full bg-[#F2F4F7] border border-[rgba(193,198,215,0.50)] rounded-full px-4 py-3 outline-none text-sm text-[#191C1E] appearance-none">
